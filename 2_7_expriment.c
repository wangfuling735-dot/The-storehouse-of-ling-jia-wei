...
void modify_student()
{
    ...
    while(fscanf(file, "%s %s %f %f %f", name, id, &chinese, &math, &english) != EOF)
    {
        if(strcmp(name, modify_Name) == 0)  // 修正：应该是modify_Name而不是deleteName
        {
            printf("已找到学生: %s %s 语文:%.2f 数学:%.2f 英语:%.2f\n",  
                   name, id, chinese, math, english);
            printf("请输入新的学生信息\n");
            printf("姓名: ");
            scanf("%s", name);
...
void performance_rank()
{
    ...
    for(int x = 0; x < i - 1; x++)
    {
        for(int y = 0; y < i - 1 - x; y++)
        {
            if(count[y] < count[y + 1])
            {
                ...
                // 交换姓名
                char temp_name[20];
                strcpy(temp_name, iname[y]);        // 修正：strcyp -> strcpy
                strcpy(iname[y], iname[y + 1]);     // 修正：strcyp -> strcpy
                strcpy(iname[y + 1], temp_name);    // 修正：strcyp -> strcpy
            }
        }
    }
    ...
    for(p = 0; p < i; p++)
    {
        printf("第%d名：姓名：%s，总分：%.2f\n", p + 1, iname[p], count[p]);  // 修正：i+1 -> p+1
    }
}
